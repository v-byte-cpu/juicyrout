...
const SlashRegexp = `(%2(f|F))`
const ProcessContentTypeRegexp = `(script|json|css)`

var (
	urlRegexp                = regexp.MustCompile(URLRegexp)
	partialURLRegexp         = regexp.MustCompile(PartialURLRegexp)
	slashRegexp              = regexp.MustCompile(SlashRegexp)
	processContentTypeRegexp = regexp.MustCompile(ProcessContentTypeRegexp)
...
func newURLRegexProcessor(log *zerolog.Logger, convertDomain convertDomainFunc) RegexProcessor {
...
func newHTMLRegexProcessor(log *zerolog.Logger, conv DomainConverter, jsHookScript string) RegexProcessor {
...
func (p *regexProcessor) ProcessAll(w io.Writer, r io.Reader) (err error) {
	...
	for {
		if err = p.Process(w, r, bufSize, buff); err != nil {
			w.Write(buff.Bytes())
			if err != io.EOF {
				p.log.Error().Err(err).Msg("io error")
			} else {
				err = nil
			}
			return
